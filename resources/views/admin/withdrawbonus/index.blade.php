...
@section('javascript')
    ...
    <script>
        function withdrawbonus(params) {
            $.ajax({
                type: "GET",
                url: "{{ route('withdrawbonus.create') }}",
                dataType: "json",
		...
		{
			var order_details = "{{ route('approvewithdraw', 'id=:user_id') }}";
            order_details = order_details.replace(":user_id", row.user_id);
			var withdraw_details = "{{ ('id=:user_id') }}";
            withdraw_details = withdraw_details.replace(":user_id", row.user_id);
			return [
