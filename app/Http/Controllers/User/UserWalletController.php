...
class UserWalletController extends Controller
{
    public function show_royalty_page()
    {
        ...
        return view('font.wallet.index',$data);
    }
    ...
    {
        $data = Wallet::join('users', 'users.user_id', '=', 'wallets.user_id')
            ->select('users.name', 'wallets.*')
            ->where('wallets.user_id', Auth::user()->user_id)->get();
    ...
    {
		$data['result']=MlmUser::select('*')->where('user_id',Auth::User()->user_id)->get();
        return view('font.wallet.wallet',$data);
    }
    ...
    {
        $data = UserCreditPoint::join('users', 'users.user_id', '=', 'redeem_credit_points.user_id')
            ->select('users.name', 'redeem_credit_points.*')
            ->where('redeem_credit_points.user_id', Auth::user()->user_id)
            ->get();
        echo json_encode($data);
    }
    ...
    {
        $result = MlmUser::select('total_pv_point')
            ->where('user_id', Auth::user()->user_id)
            ->get();
